...
type ActivityItem = {
  ...
  time: string; timestamp: number; type: ActivityType;
};
...
export default function Statistics({ onNavigate, onOpenThread }: StatisticsProps) {
  ...
  useEffect(() => {
    ...
    fetch("/api/earnings")
      ...
      .catch((e) => setEarningsError(e.message ?? "Failed to load earnings"))
  ...
  const avgPerBooking   = earnings?.avgPerBooking ?? 0;
  ...
  const revenueData: MonthPoint[] =
    monthly.length ? monthly : [{ month: "—", year: new Date().getFullYear(), amount: 0 }];
  ...
  const STATS: {
    ...
    {
      label: "Total Earned",
      value: earningsLoading ? "—" : fmt(totalEarned),
    ...
    <div className={styles.page}>
      ...
      <div className={styles.hero}>
        <div className={styles.heroText}>
          ...
          <p className={styles.heroSub}>Your complete statistics and earnings overview.</p>
        </div>
          ...
          <div className={styles.panelHead}>
            <div>
              ...
              <p className={styles.panelSub}>
                Last 6 months · Total{" "}
                <strong>{earningsLoading ? "…" : `R ${sixMonthTotal.toLocaleString()}`}</strong>
              </p>
            </div>
          </div>
                ...
                topGear.map((g) => (
                  <div key={g.listingId} className={styles.topGearRow}>
                    <div className={styles.topGearInfo}>
                      ...
                      <span className={styles.topGearEarned}>{fmt(g.earned)}</span>
                    </div>
                    <div className={styles.topGearBar}>
                      <div className={styles.topGearFill} style={{ width: `${(g.earned / topGearMax) * 100}%` }} />
                    </div>
                  </div>
                ))
              )}
      ...
      {earningsError && <p className={styles.errorMsg}>{earningsError}</p>}

      {/* ── Earnings hero ── */}
      <div className={styles.earningsHero}>
        <div className={styles.earningsHeroMain}>
          <p className={styles.earningsLabel}>Total earned (6 months)</p>
          {earningsLoading ? (
            <div className={styles.skeletonHero} />
          ) : (
            <>
              <p className={styles.earningsAmount}>{fmt(sixMonthTotal)}</p>
              {pct !== null && pct !== undefined ? (
                <p className={pct >= 0 ? styles.earningsUp : styles.earningsDown}>
                  {pct >= 0 ? "↑" : "↓"} {Math.abs(pct)}% vs previous month
                </p>
              ) : (
                <p className={styles.earningsNeutral}>No comparison data yet</p>
              )}
            </>
          )}
        </div>
        <div className={styles.earningsStats}>
          <div className={styles.earningStat}>
            {earningsLoading ? <div className={styles.skeletonStat} /> : <p className={styles.earningStatVal}>{totalBookings}</p>}
            <p className={styles.earningStatLabel}>Total bookings</p>
          </div>
          <div className={styles.earningStatDivider} />
          <div className={styles.earningStat}>
            {earningsLoading ? <div className={styles.skeletonStat} /> : <p className={styles.earningStatVal}>{fmt(Math.round(avgPerBooking))}</p>}
            <p className={styles.earningStatLabel}>Avg per booking</p>
          </div>
          <div className={styles.earningStatDivider} />
          <div className={styles.earningStat}>
            {earningsLoading ? <div className={styles.skeletonStat} /> : <p className={styles.earningStatVal}>{activeListings}</p>}
            <p className={styles.earningStatLabel}>Active listings</p>
          </div>
        </div>
      </div>
              ...
              {byListing.map((item) => (
                <div key={item.listingId} className={styles.breakdownRow}>
                  ...
                  <div className={styles.breakdownBarWrap}>
                    <div className={styles.breakdownBarFill} style={{ width: `${(item.bookings / maxBookings) * 100}%` }} />
                  </div>
                  <span className={styles.breakdownAmt}>{fmt(item.earned)}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {/* ── Lifetime total ── */}
      {!earningsLoading && totalEarned > 0 && sixMonthTotal !== totalEarned && (
      ...
      <div className={styles.twoCol}>
        <div className={styles.panel}>
          ...
          <div className={styles.timeline}>
            {activityLoading ? (
              Array.from({ length: 4 }).map((_, i) => <div key={i} className={styles.activitySkeleton} />)
            ) : (() => {
        ...
        <div className={styles.panel}>
          <div className={styles.panelHead}>
            ...
            <div className={styles.panelHeadRight}>
              {(() => {
                ...
                return unread > 0 ? <span className={styles.unreadBadge}>{unread} unread</span> : null;
              })()}
          ...
          <div className={styles.msgList}>
            {threadsLoading ? (
              Array.from({ length: 3 }).map((_, i) => <div key={i} className={styles.msgSkeleton} />)
            ) : threads.length === 0 ? (
